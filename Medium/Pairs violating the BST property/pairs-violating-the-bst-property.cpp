...
class Solution {
 ...
 int k = 0; // Global variable for indexing during in-order traversal
...
void inorder(Node* root, vector<int>& arr) {
    ...
    arr[k++] = root->data; // Place the current node's value in the array
...
int countInversion(vector<int>& arr, int l, int r) {
    if (l >= r) return 0;
    int mid = l + (r - l) / 2;
    int inv = countInversion(arr, l, mid);
...
int pairsViolatingBST(int n,Node* root) {
    k = 0; // Reset k before each call
    vector<int> arr(n); // Using vector for dynamic array
    inorder(root, arr); // Fill the array using in-order traversal
